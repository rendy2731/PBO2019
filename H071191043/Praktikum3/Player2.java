...
public class Player2 {
    ...
    public Player2 (String name, String role){
        this.name = name;
        this.role = role;
        hp = 100;
        level = 0;
        ...
        money = 100;
        items = new ArrayList<>();
        items.add(new Item2 ("Hp Potion", 20, "+ 20 Hp"));
        items.add(new Item2 ("Hp Potion", 20, "+ 20 Hp"));
    }
    ...
    public String getDamage(Player2 player){
    ...
    public boolean dead(Player2 player){
        if (player.hp <= 0) {
            return true;
        }
        else { 
    ...
    public void healing(Player2 player){
        if (!dead(player)) {
            if ((player.hp + 20) >= 100) {
                player.hp = 100;
    ...
    public void buy(Player2 player, int count){
        if(!dead(player)){
            if (money >= 20) {
